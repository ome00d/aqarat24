...
document.addEventListener('DOMContentLoaded', function() {
    // Moved Translations up for global access before init
    const translations = {
    ...
    let allPosts, currentUser, allUsers, favoritePostIds, privateProperties, passwordResetState;
    ...
    const samplePosts = [
        ...
        { id: "1726200000003", postCode: "V03", userId: "anonymous_user", listingType: "sale", propertyType: "villa", location: "ماسیک, دهۆک", priceType: "special", priceValue: "", phone: "07503334455", area: "600", direction: "روژئاڤا", bedrooms: "6", bathrooms: "5", floors: "3", garden: "yes", isZero: "yes", coords: { lat: 36.883, lng: 42.946 }, imageUrls: ["https://images.pexels.com/photos/323780/pexels-photo-323780.jpeg"], status: "pending" },
    ];
    ...
    function initializeData() {
        ...
        if (!loadData('users')) saveData('users', []);
    ...
    function applyTranslations(lang) {
        ...
        document.getElementById('users-data-search').placeholder = translations.usersSearchPlaceholder[lang];
    }
    ...
    function switchPage(pageId) {
        ...
        document.querySelector('header').style.display = isDetailPage || isMapPage || isAdminPage ? 'none' : 'flex';
        document.querySelector('.bottom-nav').style.display = isDetailPage || isAdminPage ? 'none' : 'flex'; 
        ...
        document.getElementById('profile-page').innerHTML = `
            ...
            <div id="logged-in-view" style="display: none;">
                <div class="profile-header">
                    <div id="profile-pic-container"><img src="https://via.placeholder.com/100" alt="Wêneyê Profaylê" id="profile-pic"><input type="file" id="profile-pic-input" accept="image/*"><i class="fa-solid fa-pencil" id="profile-pic-edit-icon"></i></div>
                    <h2 id="username-display"></h2>
                    <button id="edit-profile-btn" data-translate="editProfileName"></button>
                </div>
    ...
    function renderAddForm(post = {}) {
        ...
        elements.addPropertyForm.innerHTML = `
            ...
            <div class="form-notice"><i class="fa-solid fa-check-double"></i><p><strong>تێبینی:</strong> پشتی هنارتنا زانیاریێن سەرەکی، دێ تیمەکا مە سەرەدانا مولکێ هەوە کەت بۆ پشتراستبوونێ و گرتنا وێنەیێن پرۆفێشنال، و پاشی دێ پۆستێ هەوە هێتە بەلاڤکرن.</p></div>
        ...
        applyTranslations(currentLang);
        const initialMapCoords = isEditing ? (post.coords || defaultCoords) : defaultCoords;
    ...
    function handleFormSubmit(e) {
        ...
        if (isEditing) {
            ...
            if (index > -1) {
                postData.imageUrls = allPosts[index].imageUrls || []; 
                postData.status = allPosts[index].status;
                postData.userId = allPosts[index].userId;
        ...
        if (isEditing) {
            showToast(translations.postUpdatedSuccess[currentLang]);
        } else {
    ...
    function showAuthView(viewId) {
        ['login-container', 'register-container', 'forgot-password-container', 'reset-password-container'].forEach(id => {
    ...
    function handleRegistration(e) {
        ...
        if (existingUser) {
            alert('بکارهێنەرەک ب ڤی ئیمەیلی یێ هەی.');
            return;
        }
    ...
    function handleLogin(e) {
        ...
        if (!identifier || !password) {
            alert('تکایە ئیمەیل و پەیڤا نهێنی بنڤیسە.');
            return;
        }
        ...
        if (foundUser) {
            ...
            alert('ئیمەیل یان پەیڤا نهێنی خەلەتە.');
        }
    }
    ...
    function handleForgotPasswordRequest(e) {
        ...
        if (!userExists) {
            alert("هیچ بکارهێنەرەک ب ڤی ئیمەیلی نەهاتە دیتن.");
            return;
        }
        const code = Math.floor(100000 + Math.random() * 900000);
        passwordResetState = { email, code };
        alert(`کۆدێ نووکرنێ: ${code}\n(تێبینی: د بەرنامەیەکێ ڕاستەقینە دا، ئەڤ کۆدە دێ بۆ ئیمەیلێ تە هێتە هنارتن)`);
    ...
    function handlePasswordReset(e) {
        ...
        if (newPassword !== confirmPassword) {
            alert('پەیڤێن نهێنی وەکو ئێک نینن.');
            return;
        }
        ...
        saveData('users', allUsers);
        alert("پەیڤا نهێنی ب سەرکەفتیانە هاتە گوهۆڕین. نها دشێی بچیە ژوور.");
    ...
    function handleSendNotification(e) {
        ...
        alert("ئاگەهداری ب سەرکەفتی هاتە هنارتن.");
    ...
    function setupEventListeners() {
        document.querySelectorAll('.bottom-nav a').forEach(l => {
            l.addEventListener('click', e => {
                ...
                if (targetLink.id === 'fab-add-btn') {
                    if (currentUser.isAnonymous) {
                        alert("تکایە سەرەتا خۆ تۆمار بکە یان بچۆ ژوور دا بشێی پۆستەکێ زێدە بکەی.");
                        switchPage('profile-page');
                    } else {
                        switchPage('add-page');
                    }
        ...
        elements.body.addEventListener('click', function(e) {
            if (e.target.closest('#locate-me-btn')) mainMap.locate({setView: true, maxZoom: 16});
            if (e.target.closest('#zoom-in-btn')) mainMap.zoomIn();
            if (e.target.closest('#zoom-out-btn')) mainMap.zoomOut();
            if (e.target.closest('#forgot-password-link')) { e.preventDefault(); showAuthView('forgot-password-container'); }
            
            if (e.target.id === 'edit-profile-btn') {
                const newName = prompt(translations.promptNewName[currentLang], currentUser.name);
                if (newName) { currentUser.name = newName; saveData('currentUser', currentUser); renderProfilePage(); }
            }
            ...
            if(e.target.id === 'show-register-link') showAuthView('register-container');
            if(e.target.id === 'show-login-link' || e.target.id === 'back-to-login-link-1' || e.target.id === 'back-to-login-link-2') showAuthView('login-container');


             if(e.target.closest('.dev-name')) {
                adminClickCount++;
                ...
                if (adminClickCount >= 7) {
                    clearTimeout(adminClickTimer);
                    adminClickCount = 0;
                    const username = prompt("ناڤێ بکارهێنەری بنڤیسە:");
                    if (username) {
                        const password = prompt("پەیڤا نهێنی بنڤیسە:");
                        if (password) {
                            if (username.toLowerCase() === 'omed' && password === 'Omed@2008') {
                                sessionStorage.setItem('isAdmin', 'true');
                                switchPage('admin-page');
                            } else {
                                alert("ناڤ یان پەیڤا نهێنی خەلەتە!");
                            }
                        }
                    }
                }
            }
            if (e.target.id === 'admin-home-btn') {
                switchPage('home-page');
            }
            if(e.target.id === 'btn-show-posts') showAdminView('admin-posts-view');
            if(e.target.id === 'btn-show-users') showAdminView('admin-users-view');
            if(e.target.id === 'btn-show-private-data') showAdminView('admin-private-data-view');
            if(e.target.id === 'btn-show-notifications') showAdminView('admin-notifications-view');
            if(['btn-back-to-dashboard-1','btn-back-to-dashboard-2','btn-back-to-dashboard-3', 'btn-back-to-dashboard-4'].includes(e.target.id) ) showAdminView('admin-dashboard');

            if(e.target.id === 'btn-add-private-record') openPrivateDataModal();
            const modal = document.getElementById('private-data-modal');
        ...
        elements.body.addEventListener('change', function(e){
            if(e.target.id === 'profile-pic-input'){
                if (e.target.files && e.target.files[0]) { 
                    ...
                    reader.onload = e => { currentUser.pic = e.target.result; saveData('currentUser', currentUser); renderProfilePage(); }; 
    ...
    function init() {
        ...
        // Must be called last after everything is ready
